...
public class ItemAnimationFactory {
	...
	public static void animatedItems(PlayerTickEvent.Post event) {
		...
		if (mainhandItem.getItem() instanceof GeoItem || offhandItem.getItem() instanceof GeoItem) {
			if (mainhandItem.getItem() instanceof KnightSwordItem animatable) {
				...
				if (!animation.isEmpty()) {
					...
					if (event.getEntity().level().isClientSide()) {
						((KnightSwordItem) event.getEntity().getMainHandItem().getItem()).animationprocedure = animation;
					}
				}
			}
			if (offhandItem.getItem() instanceof KnightSwordItem animatable) {
				...
				if (!animation.isEmpty()) {
					...
					if (event.getEntity().level().isClientSide()) {
						((KnightSwordItem) event.getEntity().getOffhandItem().getItem()).animationprocedure = animation;
					}
				}
			}
		}
